module.exports = function initSocketModule(db, express, httpServer, rooms) {
	...
	io.sockets.on('connection', function(socket) {
		...
		log.debug('New connection: ' + socket.ip);
		...
		socket.handshake.getSession(function(err, session) {
			...
			socket.on('user::create', function(room) {
				log.debug('New user: ' + session.passport.user.username);
			...
			socket.on('ball::set', function(ball) {
				log.debug('ball::set (' + session.passport.user.username + ')');
			...
			socket.on('score::add', function(killer) {
				log.debug('score::add (' + killer + ', ' + session.passport.user.username + ')');
			...
			socket.on('point::add', function() {
				log.debug('point::add (' + session.passport.user.username + ')');
			...
			socket.on('disconnect', function() {
				...
				if (session.passport.user.isPlaying === false) {
					log.debug('Address disconnected: ' + socket.ip + '.');
				...
				log.debug('User ' + session.passport.user.username + ' left.');
				log.debug('Address disconnected: ' + socket.ip + '.');
			});
		});
	});
