module.exports = function initIndexRouterModule(config, db, app, io, rooms, swig) {
	...
	app.get('/', function(req, res) {
		...
		if (req.user) { // If user is logged in, compile rooms.
			for (var room in rooms) {
				rooms_.push({ name: room, players: io.clients(room).length, locked: !!rooms[room].password });
			}
		}
	...
	app.get('/leaderboard', function(req, res) {
		db.users.find({ username: { $exists: true, $ne: '' } }).sort({ kills: -1 }).limit(50).exec(function(err, users) {
			if (err) {
				throw err;
			}
